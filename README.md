...
**Kōdo: Settings** → **Anthropic**, **OpenAI**, **Meta**, **Google**, **Alibaba**, **DeepSeek**, or **Kimi** → add an API key, assign models to the four effort tiers, done. Keys live in VS Code's encrypted secret storage, never in a file or an environment variable, and are handed to the local server over the loopback socket at runtime. Meta's tab additionally offers an opt-in "contributor" tier — heavily discounted pricing in exchange for permission to train future Meta models on your traffic — off by default and shown with a warning (availability is country-restricted) when turned on.

Two more tabs are **aggregators**, which work a little differently: instead of a fixed lineup, they fetch their model catalogue at runtime and give you a searchable picker.

- **OpenRouter** — one API key, hundreds of models from many providers. Leave it in Auto mode and OpenRouter picks a model per request, or switch to Manual and assign a specific model to each effort tier.
- **AWS Bedrock** — models served from **your own AWS account**, billed through AWS. This one authenticates with a long-term IAM user access key, so its form asks for an access key ID *and* a secret access key, plus an AWS region (Bedrock is regional — the catalogue you get depends on it). Give the IAM user `bedrock:InvokeModelWithResponseStream`, `bedrock:ListFoundationModels`, and `bedrock:ListInferenceProfiles`, and enable the models you want in the Bedrock console. Kōdo lists cross-region inference profiles alongside plain model ids, since many Bedrock models can only be invoked on demand through one. Bedrock reports no per-call cost, so usage there shows tokens without a dollar figure.

That's nine cloud vendors wired up today, and none of the tabs is a placeholder.
...
3. **More cloud providers.** Nine are wired up, including the OpenRouter and AWS Bedrock aggregators; the list isn't closed.
