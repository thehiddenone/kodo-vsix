...
Anthropic, OpenAI, Meta, Google, Alibaba, DeepSeek, and Kimi are the seven cloud vendors wired up today. The other tab — OpenRouter — exists in the settings panel as an honest "coming soon" placeholder rather than as a working integration pretending otherwise.
...
3. **More cloud providers.** OpenRouter as an aggregator — the settings tab is already waiting for it.
